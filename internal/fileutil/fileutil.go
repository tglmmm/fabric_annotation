...
// renames the tmpFile to the finalFile. In other words, in the event of a crash,
// either the final file will not be visible or it will have the full contents.
// The tmpFile should not be existing. The finalFile, if exists, will be overwritten (default rename behavior)
// 将内容写入到临时文件，文件系统层面同步一下tempFile,将临时文件重命名为finalFile,在系统崩溃情况向
// 最终文件要吗不可见，要吗就是具有完整的内容
// 临时文件不应该存在，finalFile将被覆盖
