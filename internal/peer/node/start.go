...
func serve(args []string) error {
	...
	// 目前该peer节点只支持标准的MSP
	// 因为在某些情况下，msp必须确保他来自一个单一的证书，你仅仅只能有一个身份
	// idemix 没有支持，但是很容易做到
	// 现在只能确保使用MSP

	// 获取MSP类型
	//参数是加密方法，得到 ProviderType(标识提供者类型)
	// 从bccsp 获取 localMSP
	...
	// 是否使用 golang.org/x/net/trace 跟踪RPC,在rpc发起或者接收之前设置
	...
	aclProvider := aclmgmt.NewACLProvider(
		// GetStableChannelConfig 返回已经创建的通道的配置信息
	...
	// FIXME: Creating the gossip service has the side effect of starting a bunch of go routines and registration with the grpc server.
	// 创建一个gossip 服务的副作用就是启动一堆 goroutine 向服务注册
...
// 创建链码服务 ，使用配置文件中chaincodeListenAddress指定的地址端口，如果没有设置则使用peerAddress
