...
func ConfigPaths() []string {
	var paths []string
	// 从环境变量读取 配置文件所在的目录$FABRIC_CFG_PATH
	if p := os.Getenv("FABRIC_CFG_PATH"); p != "" {
		paths = append(paths, p)
	}
	// 总共有三个地方查找配置文件，环境变量， 当前目录，/etc/hyperledger/fabric
...
type ConfigParser struct {
	...
	configPaths []string // 配置文件存在的路径列表
...
func (c *ConfigParser) ReadConfig(in io.Reader) error {
	// 解码yaml 配置文件到map[string]interface{}
...
func getKeysRecursively(base string, getenv envGetter, nodeKeys map[string]interface{}, oType reflect.Type) map[string]interface{} {
	...
	// 再次查验 otype 不能是Nil ,且是一个结构体
	if oType != nil && oType.Kind() == reflect.Struct {
	outer:
		// 遍历结构体字段
		for i := 0; i < oType.NumField(); i++ {
			fieldName := oType.Field(i).Name // 字段名称
			fieldType := oType.Field(i).Type // 字段类型
			// 遍历map[string]interfaces
			// 这一步操作实际上是给变量设置具体的类型而不需要通过逐个的类型断言去处理
			for key := range nodeKeys {
				if strings.EqualFold(fieldName, key) {
					// map[key] =reflect.Type
					// 将配置中和具体的结构体相同的 键与字段类型做映射
					subTypes[key] = fieldType
					continue outer
				}
			}

			//fmt.Printf("subTypes[%s] = %v\n", fieldName, fieldType)
			// 如果 配置文件中不存在的字段，则在这里进行处理
	...
	for key, val := range nodeKeys {
		...
		case map[string]interface{}:
			// 如果是 map[string]interface{} 继续进行递归
...
// 将 map[interface]interface 转换成 map[string]interface{}
...
// producing error when extraneous variables are introduced and supporting the time.Duration type
// 意在反序列化配置文件中的内容到结构体
// 当引入外部变量和支持 time.Duration 类型时候产生错误
func (c *ConfigParser) EnhancedExactUnmarshal(output interface{}) error {
	...
	// config是yaml配置文件decode后的值map[string]interface{}
	baseKeys := c.config
