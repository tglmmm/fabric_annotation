...
// 获取到一个文件锁，通过给定的路径打开数据库来获取，在内部leveldb获取一个文件锁当打开一个db时候，如果数据库被再一次的被同一个或者其他进程打开
// 就会返回一个错误，当数据库被关闭或者 拥有者进程死亡，这个锁将会被释放，并且其他进程可以打开数据库，我们利用Leveldb释放获取文件锁的功能适用于其他操作系统中
