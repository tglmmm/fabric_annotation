...
func InitCrypto(mspMgrConfigDir, localMSPID, localMSPType string) error {
	...
	// 将默认的bccsp opts 选项覆盖到配置文件中BCCSP部分
...
// 为bccsp 设置keystore 路径
func SetBCCSPKeystorePath() {
	// 在配置文件中读取，如果没有设置默认是 mspConfigPath/keystore
...
func InitCmd(cmd *cobra.Command, args []string) {
	// 设置配置文件路径（FABRIC_CFG_PATH）已经配置文件名称
	...
	// 如果是链码打包子命令或者计算链码packageId 不需要加载本地MSP，所以直接返回
