...
// 创建MSP实例通过BCCSP，可以生成身份和签名身份，通过证书和密钥对
...
func (msp *bccspmsp) GetType() ProviderType {
	fmt.Println("peer node start get MSP Type")
	return FABRIC
}
...
func (msp *bccspmsp) getUniqueValidationChain(cert *x509.Certificate, opts x509.VerifyOptions) ([]*x509.Certificate, error) {
	...
	// 我们仅仅支持一个简单的 认证链 ， 如果有超过一个，可能就不清除谁拥有这个身份
...
// 如果签名证书包含名称约束，子证书没有包含SAN的扩展，并且叶子证书看起来是一个主机名，
// 则验证失败，
...
// sanitizeCert ensures that x509 certificates signed using ECDSA do have signatures in Low-S. If this is not the case,
// the certificate is regenerated to have a Low-S signature.
// 确定证书是否用ECDSA椭圆加密算法签名,有Low-S签名
// 重新生成证书并且含有Low-S签名
