...
type OrganizationalUnitIdentifiersConfiguration struct {
	...
	// 组织单元标识 peer , client ,orderer ,admin
...
// NodeOUs 包含如何区分 client, peer ,orderer 的信息，如果强制执行检查，设置Enable: true即可
// 如果身份是 client, peer , orderer MSP则认为身份有效
type NodeOUs struct {
	// Enable activates the OU enforcement
	Enable bool `yaml:"Enable,omitempty"` // 是否启用
	...
	// OrdererOUIdentifier specifies how to recognize orderer by OU
	// 指定识别
...
//returns a local MSP configuration for the MSP in the specified directory, with the specified ID and type
// 返回指定MSP目录中MSP配置，通过指定的ID和类型
func GetLocalMspConfigWithType(dir string, bccspConfig *factory.FactoryOpts, ID, mspType string) (*msp.MSPConfig, error) {
	switch mspType {
	case ProviderTypeToString(FABRIC): //bccsp
		return GetLocalMspConfig(dir, bccspConfig, ID)
	case ProviderTypeToString(IDEMIX): // idemix
