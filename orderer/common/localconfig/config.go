...
var Defaults = TopLevel{
	General: General{
		...
		// 如果不指定创世文件，默认在配置目录下查询名称为 genesisblock文件
		BootstrapFile: "genesisblock",
		// pprof http接口查看go程序的运行以及资源水平
		Profile: Profile{
			Enabled: false,
...
// 加载解析orderer.yaml配置文件和环境变量，产生一个结构体用于配置
...
func (c *configCache) load() (*TopLevel, error) {
	...
	// c.cache[orderer.yaml]
	// 查看配置文件是否已经被缓存
	serializedConf, ok := c.cache[config.ConfigFileUsed()]
	if !ok {
		// 如果缓存中不存在配置，将配置信息解码到orderer配置文件对应结构体uconf
...
func (c *TopLevel) completeInitialization(configDir string) {
	// 将配置中的证书，创世块，账本目录中的相对路径修改为绝对路径，规则是以配置文件目录作为根目录
	defer func() {
		...
		if c.General.Cluster.ClientPrivateKey != "" {
			// 如果私钥文件不是绝对路径，将路径转换为绝对路径，配置文件和证书路径在同一个路径下
