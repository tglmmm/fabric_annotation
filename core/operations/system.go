...
type System struct {
	*fabhttp.Server
	...
	collectorTicker *time.Ticker // 收集定时器
	sendTicker      *time.Ticker // 发送定时器
...
func NewSystem(o Options) *System {
	...
	system.initializeHealthCheckHandler() // 初始化健康检查相关的handler
	system.initializeLoggingHandler()     //  初始化日志规格
