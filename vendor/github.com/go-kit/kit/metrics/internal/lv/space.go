...
// Space represents an N-dimensional vector space. Each name and unique label value pair establishes a new dimension and point within that dimension.
//Order matters, i.e. [a=1 b=2] identifies a different timeseries than [b=2 a=1].

// 表示一个N维的向量空间，每个名称和唯一的标签值组成的对作为一个新的维度和该维度中的点
type Space struct {
	mtx   sync.RWMutex
